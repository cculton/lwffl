...
const LWFFL = (() => {
  ...
  const resolveManagerSlug = (slug, managers) => {
    if (Object.prototype.hasOwnProperty.call(managers, slug)) return slug;
    const legacy = String(slug || "").match(/^([a-z]+)-([a-z]{2,})$/);
    if (!legacy) return null;
    const matches = Object.values(managers).filter(manager => {
      const [first, initial] = slugify(manager.name).split("-");
      return first === legacy[1] && (!initial || initial === legacy[2][0]);
    });
    return matches.length === 1 ? matches[0].slug : null;
  };

  /* First names are sufficient except where the Ryans need differentiation. */
  ...
  return { load, slugify, resolveManagerSlug, shortName, tenureStr, fmt, fmtInt, mlink, alumPill, managerCell, managerLabel, finishCell, ylink, wlink, bxlink, bxHref, hasBox, recordStr, sparkline, isPlayoff };
})();
